      ...
      <el-table-column prop="tenantName" label="所属组织" min-width="210" align="center">
        <template slot-scope="scope">
          <el-popover placement="right" trigger="hover" v-if="scope.row.tenantName.split(',').length > 1">
            <p
              v-for="(item,index) in scope.row.tenantName.split(',')"
              :key="index"
      ...
      <el-table-column prop="deptName" label="所属部门" min-width="110" align="center">
        <template slot-scope="scope">
          <el-popover placement="right" trigger="hover" v-if="scope.row.deptName.split(',').length > 1">
            <p
              v-for="(item,index) in scope.row.deptName.split(',')"
              :key="index"
              style="line-height: 10px;"
            >
              {{item}}
          ...
          <el-col :span="12">
            <el-form-item label="所属组织" prop="tenantName">
              <el-popover placement="bottom" trigger="hover"
                ...
                <p
                  v-for="(item,index) in ruleForm.tenantName.split(',')"
                  :key="index"
                  style="line-height: 10px;"
                >{{item}}</p>
                <el-input
          ...
          <el-col :span="12">
            <el-form-item label="所属角色" prop="roleName">
              <el-popover placement="bottom" trigger="hover"
                ...
                <p
                  v-for="(item,index) in ruleForm.roleName.split(',')"
                  :key="index"
                  style="line-height: 10px;"
                >{{item}}</p>
                <el-input
          ...
          <el-col :span="12">
            <el-form-item label="所属部门" prop="deptName">
              <el-popover placement="bottom" trigger="hover"
                ...
                <p
                  v-for="(item,index) in ruleForm.deptName.split(',')"
                  :key="index"
                  style="line-height: 10px;"
                >{{item}}</p>
                <el-input
    ...
    methods: {
      ...
      handleDevice2() {
        ...
        getAllDeviceList(this.selection[0].id).then(res => {
          ...
          let checkDeviceList = []
          for (let i = 0; i < res.data.data.checkDeviceList.length; i++) {
