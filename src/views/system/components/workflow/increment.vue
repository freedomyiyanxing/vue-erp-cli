  ...
  <basic-container>
    ...
    >
      ...
      <div class="box">
        <div @click="initiatorClick" class="box-card pointer">
          ...
          <div class="box-card-body">
            <span v-if="!form.deptId"> 请选择发起人部门/角色 </span>
            <span v-if="form.deptId"> {{ form.deptName }}/{{ form.roleName }} </span>
          </div>
        </div>

        <div>
          <i @click="addRight(0)" class="el-icon-circle-plus-outline iconfont pointer" style="font-size: 30px" />
        </div>
        ...
        >
          <i
            @click="delApprove(index)"
            class="pointer iconfont el-icon-circle-close del"
            v-if="index > -1 && form.subProcessList[0].processTaskList.length !== 1"
          />
          <div @click="approveClick(i, index)" class="box-card pointer">
            ...
            <div class="box-card-body">
              ...
              <el-tooltip :content="i.policeName" class="item" effect="dark" placement="top" v-if="i.policeId">
                <el-button size="mini">指定人员</el-button>
              </el-tooltip>
            </div>
          </div>
          <i
            @click="addRight(index + 1)"
      ...
      <el-card class="card" v-if="isInitiator">
        ...
        <div style="margin-bottom: 10px">
          <el-select
            ...
            value-key="id"
          >
            <el-option :key="item.id" :label="item.deptName" :value="item" v-for="item in options"> </el-option>
          </el-select>
        </div>
        <div>
          <el-select
      ...
      <el-card class="card" v-if="isApprove">
        <div slot="header">
          <span style="font-weight: bold">设置审批人{{ approveIndex + 1 }}</span>
        </div>
        <div>
...
export default {
  ...
  methods: {
    ...
    addRight(index) {
      this.form.subProcessList[0].processTaskList.splice(index, 0, {
        assignType: 1,
        policeId: '', // 3
        postId: '', // 1,2
    ...
    sure() {
      ...
      const a = this.form.subProcessList[0].processTaskList.filter((i) => !(i.postId + i.policeId));
      if (a.length !== 0) {
        this.$message({
          type: 'error',
          message: '审批人必填!',
