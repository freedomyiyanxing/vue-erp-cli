  ...
  <el-form :model="form" class="demo-ruleForm" inline ref="form" size="mini">
    <el-form-item :key="data.prop" v-for="(data, index) in configs.condition || []">
      ...
      <el-select
        ...
        clearable
        :disabled="data.disabled || false"
        @change="(e) => handleChange(e, index, data)"
        :style="`width:${data.width || 200}px;`"
        v-if="data.element === 'el-select'"
        v-model="form[data.prop]"
      >
        <el-option
      ...
      <el-autocomplete
        ...
        :disabled="data.disabled || false"
        :placeholder="data.placeholder"
        @focus="(e) => autocompleteFocus(e, index)"
        @select="(e) => handleSelect(e, index)"
        clearable
        :style="`width:${data.width || 200}px`"
        v-if="data.element === 'el-autocomplete'"
        v-model="form[data.prop]"
      >
      ...
      <el-input
        :placeholder="data.placeholder"
        :disabled="data.disabled || false"
        v-if="data.element === 'el-input-select' && data.isPermission"
        v-model="form[data.prop]"
      >
        <el-select
          placeholder="请选择"
          slot="prepend"
          :style="`width:${data.width || 100}px`"
      ...
      <!-- el-date-picker -->
      <el-date-picker
        v-if="data.element === 'el-date-picker'"
        v-model="form[data.prop]"
...
export default {
  ...
  created() {
    // 初始化权限都为真
    if (this.configs.butList) this.configs.butList.map((item) => ({ ...item, isPermission: true }));
    if (this.configs.condition) this.configs.condition.map((item) => ({ ...item, isPermission: true }));
  },
  // 超过2级路由的页面
  mounted() {
    this.isExecute = true;
  ...
  methods: {
    ...
    calculateWidth(formWidth) {
      if (this.configs.condition) {
        ...
        this.list = [];
        let sum = 0;
        for (let i = 0; i < this.conditionList.length; i += 1) {
          const width = this.conditionList[i].width || 210;
          sum += width;
          if (sum > formWidth) {
            this.isUnfold = true;
      ...
      this.configs.condition.forEach((item, index1) => {
        if (item.element === 'el-autocomplete' && index1 === this.index) {
          ...
          for (const key in item.params) {
            ...
            getData(item.axiosUrl, item.params)
              .then((res) => {
                cb(res.data.data);
              })
              .catch(() => {
    ...
    handleChange(val, index, item) {
      this.configs.condition.forEach((item1) => {
        // 处理时间控件，可选择和禁止
        if (item.isRelevance === item1.prop) {
          // 找到关联的下拉和时间控件
    ...
    onSubmit(val) {
      if (val.prop === 'reset') {
        // 重置
        this.form = {};
        this.configs.condition.map((item) => {
