...
// 检验库存
export const verificationStock = async (data) => new Promise((resolve) => {
  request({
...
// 新增入库提交
export const postAddSalesOrder = async (url, data) => new Promise((resolve) => {
  request({
    url,
    method: 'post',
    data,
...
// 更改订单状态
export const dispatchSalesOrderStatus = async (data) => new Promise((resolve) => {
  request({
...
// 销售订单全部解锁
export const dispatchSalesUnlock = (data) => new Promise((resolve) => {
  request({
...
export const getBuyerList = async (params) => {
  const { data } = await request.get('/api/oksht-data/buyerGroupConfig/drpGetBuyerByBrand', {
    params,
  }).catch(() => {
    return { data: { data: [] } };
  })
